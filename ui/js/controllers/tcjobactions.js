...
treeherder.controller('TCJobActionsCtrl', function($scope, $http, $uibModalInstance,
    ...
    $scope.triggerAction = function() {
        ...
        task.payload.env['ACTION_TASK_ID'] = JSON.stringify(originalTaskId);

        $http.get(`https://queue.taskcluster.net/v1/task/${originalTaskId}`).then(function(response) {
    ...
    if (decisionTaskGUID) {
        ThJobDetailModel.getJobDetails(
            { job_guid: decisionTaskGUID, title: 'artifact uploaded', value: 'actions.json'},
            {timeout: null}).then(function(details) {
                if (!details.length) {
